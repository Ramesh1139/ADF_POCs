...
// ---------------------------------------------------------------------
public class TciDesVORowImpl extends GenericViewRowImpl {
    ...
    public Boolean calculateDisabled(String attrName) {
        ...
        if (!disabled) {
            if (ADFContext.getCurrent().isJEE()) {
                ...
                String cimsStscod =
                    (String) disabledControlModel.getData("TCI_MST:" + this.getCidsDivcod() + "|" + this.getCidsRunnum().toString(), "CimsStscod");
                if (cimsStscod.equalsIgnoreCase("DE"))
                    return true;
                else {
                    if (attrName.equalsIgnoreCase("CidsRm2des")) {
                        if (this.getRm2desLbl().equalsIgnoreCase("Optional Remarks")) {
                            return true;
                        }
                    }
                }
    ...
    public static final int RM2DESLBL = AttributesEnum.Rm2desLbl.index();

    /**
     * This is the default constructor (do not remove).
     */
    public TciDesVORowImpl() {
    }
    ...
    /**
     * Gets the attribute value for the calculated attribute Rm2desLbl.
     * @return the Rm2desLbl
     */
    public String getRm2desLbl() {
        if (ADFContext.getCurrent().isJEE()) {
            DisabledLogicContainerModel disabledControlModel = (DisabledLogicContainerModel) ADFContext.getCurrent()
                                                                                                       .getPageFlowScope()
                                                                                                       .get("XTSDisabledLogicContainerBean");

            String j60 = (String) disabledControlModel.getData("TCI_MST:" + this.getCidsDivcod() + "|" + this.getCidsRunnum().toString(), "opJ60");
            String opJ60;
            if (j60 == null) {
                XTSSalInvAMImpl am = (XTSSalInvAMImpl) this.getViewObject().getApplicationModule();
                TciMstVORowImpl mstRow = (TciMstVORowImpl) am.getTciMstVO().getCurrentRow();
                String cuscod = mstRow.getCimsCuscod();
                opJ60 = am.checkSetcod(this.getCidsDivcod(), cuscod, "J60");
                disabledControlModel.addData("TCI_MST:" + this.getCidsDivcod() + "|" + this.getCidsRunnum().toString(), "opJ60", opJ60);
            } else {
                opJ60 = j60;
            }

            if ((opJ60 != null) && (!opJ60.equalsIgnoreCase("N"))) {
                return opJ60;
            }
        }
